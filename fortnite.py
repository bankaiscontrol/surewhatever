...
async def main() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.request(
            ...
            invite = (await r.json())['invite'] if r.status == 200 else "RW9spUtEnx"

    print(crayons.cyan(f"[Credits] [{datetime.datetime.now().strftime('%H:%M:%S')}] Made By SmackW "
                       'Massive credit to Terbau for creating the library.'))
    print(crayons.cyan(f"[Server] [{datetime.datetime.now().strftime('%H:%M:%S')}] Roblox Da Hood Copy Game"
                       f"https://discord.gg/{invite} - To Play The Game ETC"))
